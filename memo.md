...
- 기존에 훅에서 스테이트, ref정의 후 훅을 사용하는 최상위 컴포넌트에서 임포트 하는 방식은 스테이트, ref관리가 힘든 느낌이 들어서 => 최상위 컴포넌트에서 state, ref정의 후 훅쪽으로 파라미터로 내려주는
  ...
  의 형태로 배열안에 객체 형태로 넣어서 넣은 값은 객체 형태로 보관(utilFunction에 공통 함수로 저장 함수 정의) / 배열과 객체는 참조값을 동일하며, state의 문자열같은 원시타입은 스테이트 변경 시 
  리랜더링 되는 원리로 인해 최상위 컴포넌트에서 커스텀 훅 호출 시 최신 상태의 ref, 스테이트를 커스텀 훅에서 사용 가능 / 
