...
function useMenuSearchBusiness(data, states,refs){
    ...
    // 날짜 mm.dd로 자르기
    function today_format(month = 0, day = 0){
        const today = new Date()

        function format_number(date){
            return date < 10 ? '0' + date : date
        }



        let new_month = today.getMonth() + month
        let new_day = today.getDate() + day

        const new_date = new Date(today.getFullYear(), new_month, new_day)
        
        return `${format_number(new_date.getMonth() + 1)}.${format_number(new_date.getDate())}`
    }

    return {searchBtn , today_format}
}
