...
function LinkBtn({text, url, fetch_handler, drop_data, index, button_state}){
    // =================================================
    // hooks //
    // business
    ...
    return(
        <div className="link-btn__container">
            <button  style={{display : `${!url ? 'none' : 'block'}`}} disabled={button_style(url, button_state, text, false)} 
            className={`link-btn ${button_style(url, button_state, text, true)}`} 
