...
function Conditional_router({data_state, element : Element, redirection_url}){
    ...
    return(
        ...
        user_data.user_data.host_state ? <Element login_user = {user_data.user_data}/> : 
        <Navigate to={`${redirection_url}?name=${user_data.user_data.userId}&host=${user_data.user_data.host_state ? user_data.user_data.host_state : 'none'}`}/>
        : <Navigate to="/Login"/>
    )
}
