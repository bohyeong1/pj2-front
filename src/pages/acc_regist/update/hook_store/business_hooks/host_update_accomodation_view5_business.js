...
function useHostUpdateAccomodationView5Business(data, states, refs, props){
    ...
    function click_box(id){
        const sellect = default_data.d_keyword
        for(let i = 0; i < sellect.length; i++){
            if(i == id){
                keyword.current[id].classList.toggle('acc-regist-sellect-box-active')
            }
        }
    ...
    useEffect(()=>{
        if(is_button){
            if(current_data.length === acc_data.keywords.length && _.every(acc_data.keywords,(el)=>{
        ...
        if(!is_button && current_data.length > 0 && current_data.length !== acc_data.keywords.length){
            setIs_button(true)
            return
        }
    },[current_data])
    ...
    async function fetch_acc(){
        setLoading(false)
        const acc_data = await connect_data_width_cookies(`${default_data.d_base_url}/api/accomodation/modify/keyword/${param.house}`, 'PUT', 
            {
                keywords : current_data
            })
...
export default useHostUpdateAccomodationView5Business