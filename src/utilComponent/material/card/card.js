...
function Card({data, custom_overlay}){
    ...
    // style
    const {custom_overlay_click, custom_overlay_hover, custom_overlay_out} = useMaterialCardStyle(  
        {
            'map_state' : map_state,
            'list_hover_state' : list_hover_state
        },
    undefined, undefined,
        {
    ...
    return(
        <div className="card-container">
            <div className={`card-title ${map_state === data._id ? 'card-title__active' : ''} ${!map_state && list_hover_state === data._id ? 'card-title__hover' : ''}
            ${map_state !== data._id ? 'card-title__hover-on' : ''}`}
            data-key={data._id} onClick={custom_overlay_click} onMouseEnter={custom_overlay_hover} onMouseLeave={custom_overlay_out}>
            ...
            <div className={`card-contents ${map_state === data._id ? 'card-contents__active' : ''}`}>
                <div className="card-contents__img">
                    <Static_img url={data?.main_img}></Static_img>
                </div>
