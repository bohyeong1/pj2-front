...
function useButtonPricebtnBusiness(data, states, refs, props){
    ...
    // refs
    const {price_target, price_thumb, price_track} = refs
    ...
    function mouse_down(e){
        ...
        if(e.currentTarget.dataset.drag === 'false' && e.target.classList.contains('pricebtn-thumb')){
            ...
            const key_class = e.target.className.split(' ')[1]
            // console.log(key_class)
            e.currentTarget.dataset.target = key_class
        }
    }
    ...
    function mouse_up(e){
        if(price_target.current.dataset.drag === 'true'){
            ...
            const target = price_target.current.dataset.target

            // 현제 인덱스 값 조회
            const index = target ==='pricebtn-min' ? price_target.current.dataset.min_index : price_target.current.dataset.max_index

            if(target === 'pricebtn-min' && states.index_min_state !== index){
                states.setIndex_min_state(parseInt(index))
                // 파라미터 변경
                SearchParams.set(keyValue1, value_collections[index] +'%min')
                setSearchParams(SearchParams)
            }
            if(target === 'pricebtn-max' && states.index_max_state !== index){
                states.setIndex_max_state(parseInt(index))
                // 파라미터 변경
    ...
    function mouse_drag(e){
        if(price_target.current.dataset.drag === 'true'){
            const target_class = price_target.current.dataset.target
            ...
            if(target_class === 'pricebtn-min' && filtered_index >= states.index_max_state){
                return
            }
            if(target_class === 'pricebtn-max' && filtered_index <= states.index_min_state){
                return
            }


            // 박스 이동
            move_thumb(filtered_index, target_class)
    ...
    function move_thumb(index, target){
        const box_width = price_thumb.current.offsetWidth
        // console.log(box_width)
        const length = value_collections.length - 1
        const percentage = target === 'pricebtn-max' ? (index - length) / length : index / length
        const width = price_target.current.offsetWidth
        ...
        if(index > 0 && index < 7){
            gsap.to(`.${target}`,{
                x:target === 'pricebtn-max' ? position + box_width/2 : position - box_width/2,
                duration: 0
            })
        }
        // track 처음
            ...
            gsap.to(`.${target}`,{
                x:target === 'pricebtn-max' ? position + box_width : position,
                duration: 0
            })
        }
        // track 끝지점
            ...
            gsap.to(`.${target}`,{
                x:target === 'pricebtn-max' ? position : position - box_width,
                duration: 0
            })
        }
        ...
        // range 범위 색상 설정
        const range_min = target === 'pricebtn-min' ? (index / length)*100 : (states.index_min_state / length)*100
        const range_max = target === 'pricebtn-max' ? (index / length)*100 : (states.index_max_state / length)*100
    ...
    function mouse_click(e){
        ...
        const target_class = 
        Math.abs(states.index_min_state - filtered_index) > Math.abs(states.index_max_state - filtered_index) ? 'pricebtn-max' : 'pricebtn-min'

        // 타겟의 인덱스
        const target_index = target_class === 'pricebtn-max' ? states.index_max_state : states.index_min_state
        ...
        // 파라미터 변경 & 스테이트 변경
        if(target_class === 'pricebtn-min' && states.index_min_state !== filtered_index){
            states.setIndex_min_state(parseInt(filtered_index))
            // 파라미터 변경
            SearchParams.set(keyValue1, value_collections[filtered_index] +'%min')
            setSearchParams(SearchParams)
        }
        if(target_class === 'pricebtn-max' && states.index_max_state !== filtered_index){
            states.setIndex_max_state(parseInt(filtered_index))
            // 파라미터 변경
    ...
    },[states.index_max_state, states.index_min_state])
