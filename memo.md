...
8.3
- 기존에 훅에서 스테이트, ref정의 후 훅을 사용하는 최상위 컴포넌트에서 임포트 하는 방식은 스테이트, ref관리가 힘든 느낌이 들어서 => 최상위 컴포넌트에서 state, ref정의 후 훅쪽으로 파라미터로 내려주는
  로직으로 변경
  파라미터로 스테이트, ref 내려줄 때 함수 파라미터 입력 시 가시성을 위해서 
    ...example...
    state_store([
        {
  ...
  ********* 컴포넌트 안에서 사용하는 state, ref는 정의해 둔 공통 함수의 store로 관리 => 훅을 사용하는 최상위 컴포넌트의 라이프 사이클과 동일
  ********* 다른 컴포넌트에서도 사용하여 라이프사이클을 조정할 필요가 있는 state => redux로 관리
...
- 모달창 클릭 시 뒷배경 어둡게 만드는 overay생성 시 position:fixed 속성 줄 때 부모 요소에 filter,transform과 같은 속성 들어가 있을 시
  ...
  부모 요소 position : absolute, 가운데 정렬 할 시 transform:translate(-50%, -50%)보단 margin:auto로 가운데 정렬할 것

  하나의 overay로 모든 모달/드랍다운에 사용하기 위해서 리덕스 툴킷으로 상태관리 및 드롭다운/모달창 열닫기 제어하는 방식으로 변경
  
8.5
- 서브페이지 전체 데이터 패치 후 프론트에서 필터 -> 페이지 네이션 한 페이지에 들어가는 list만큼만 패치하는 방식으로 변경
