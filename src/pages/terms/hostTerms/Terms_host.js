...
function Terms_host(){
    ...
    return(
        <div className="Terms_host-container">
            <Main_menu></Main_menu>
        
        
            <div className="Terms_host-content">
                <Pri_side_menu data={default_data.terms_sidemenu}></Pri_side_menu>
        
                <div className="Terms_host-con-main">
                    <div className="Terms_host-con-main-title">홈페이지 약관</div>
        
                    <div className="Terms_host-con-main-sec1">
                        {default_data.host_terms.map((el, id)=>{
                            return(
                                <div className="Terms_host-c-m-s1-b">
                                    <div className="Terms_host-c-m-s1-b-d" onClick={()=>{clickBtn(id)}}>
                                        <span>{el.title}</span>
                                        <img ref={(el)=>{terms_s_btn.current[id] = el}} src={default_data.d_imgs.down_arrow} state = {false}
                                       ></img>
                                    </div>

                                    <div className="Terms_host-c-m-s1-b-d2" ref={(el)=>{terms_s_text.current[id] = el}}>{el.text}</div>
                                </div>
                            )
                        })}
                    </div>
            ...
            <div className="Terms_host-footer">
                <Footer></Footer>
            </div>
         </div>
    )
